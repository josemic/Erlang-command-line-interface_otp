...
%%% Really for test puroses only:
    Show_number_fun = fun(VTY, Command_param)-> 
				  ...
				  []-> 
				      ...
				      service_registration_lib:vty_out(VTY, "%% no number was given~n", []),
				      cmd_success;
				  [Nr0]  -> 
				      %% one number was given
				      service_registration_lib:vty_out(VTY, "%% number 0: ~p was given'~n", [Nr0]),
				      cmd_success;
				  [Nr0,Nr1] ->
				      %% two numbers were given
				      service_registration_lib:vty_out(VTY, "%% number 0: ~p and number 1: ~p were given~n", [Nr0, Nr1]),
				      cmd_success
			      end
		      end, 
...
%%% Really for demonstration puroses only:
    Show_selection_fun = fun(VTY, Command_param)-> 
				 ...
				 case SelectionList of 
				     [Selection]  -> 
					 %% one number was given
					 service_registration_lib:vty_out(VTY, "%% Selection: ~p was given'~n", [Selection]),
					 cmd_success
				 end
			 end, 
   ...
   Aliastest_cmd = #command{alias_ref= show_net_alias_def,
    ...
    service_registration_lib:install_node(demo_node, 
    ...
    %% Enter demo_node
    service_registration_lib:install_element([gsmnet_node], Cfg_demo_cmd),
    service_registration_lib:install_element([demo_node], Show_a_cmd),
    service_registration_lib:install_element([demo_node], Show_b_cmd),
    service_registration_lib:install_element([demo_node], Show_c_cmd),
    service_registration_lib:install_element([demo_node], Show_d_cmd),
    service_registration_lib:install_element([demo_node], Show_e_cmd),
    service_registration_lib:install_element([demo_node], Show_f_cmd),   
    service_registration_lib:install_element([demo_node], Show_selection_cmd),
    service_registration_lib:install_element([demo_node], Aliastest_cmd).
