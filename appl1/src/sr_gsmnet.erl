...
install()->    
    ...
    Show_bts_fun = fun (VTY, Command_param) ->
			   ...
			   case NumberList of 
				   ...
				   service_registration_lib:vty_out(VTY, "%% can't find BTS '~p'~n", [BTS_NR]),
				   cmd_warning;
			       [BTS_NR]  -> 
				   dump_bts(VTY,BTS_NR),
    ...
    Cfg_net_name_short_fun =  fun(VTY, Command_param)->
				      ...
				      service_registration_lib:vty_out(VTY,  "short network name ~p ~n", [ets:lookup_element(gsmnet_table,name_short, 2)]),
				      cmd_success
			      end,
    Cfg_net_name_short_basicwrite_fun = fun(VTY) ->
						NameShort = ets:lookup_element(gsmnet_table,name_short, 2),   
						service_registration_lib:vty_out(VTY,  "short name ~s ~n", [NameShort]) 
					end,
    ...
    Cfg_net_name_long_fun =  fun(VTY, Command_param)->
				     ...
				     service_registration_lib:vty_out(VTY,  "long network name ~p ~n", [ets:lookup_element(gsmnet_table,name_long, 2)]),
			             cmd_success
			     end,

    Cfg_net_name_long_enhancedwrite_fun = fun(VTY) ->
						  NameLong = ets:lookup_element(gsmnet_table,name_long, 2),   
						  service_registration_lib:vty_out(VTY,  "~s ~n", [NameLong])  
					  end,
    ...
    Gsmnet_node_entry_fun = fun(VTY) ->
					service_registration_lib:vty_out(VTY,  "network~n")
					%% CountryCode = ets:lookup_element(gsmnet_table,country_code, 2),   
					%% service_registration_lib:vty_out(VTY,  "network country code ~p ~n", [CountryCode]),  
					%% NameShort = ets:lookup_element(gsmnet_table,name_short, 2),
					%% service_registration_lib:vty_out(VTY,  "short name ~p~n", [NameShort])
				end,

    service_registration_lib:install_node(gsmnet_node, 
    ...
    initialize_gsmnet_data(),
    %% Enter gsmnet_node
    service_registration_lib:install_element([config_node], Cfg_net_cmd),
    service_registration_lib:install_element([gsmnet_node], Cfg_net_ncc_cmd),
    service_registration_lib:install_element([gsmnet_node], Cfg_net_name_short_cmd), 
    service_registration_lib:install_element([gsmnet_node], Cfg_net_name_long_cmd),
    service_registration_lib:install_element([gsmnet_node], Cfg_trx_arfcn_cmd),
    service_registration_lib:install_element([gsmnet_node], Cfg_bts_lac_cmd),
    %% should be config node instead of gsmnet_node	
    service_registration_lib:install_element([enable_node, view_node], Show_net_cmd),
    service_registration_lib:install_element([enable_node, view_node], Show_bts_cmd),
    service_registration_lib:install_element([enable_node, view_node], Show_trx_cmd),
    service_registration_lib:install_element([enable_node, view_node], Show_ts_cmd),
    service_registration_lib:install_element([enable_node, view_node], Show_lchan_cmd),
    service_registration_lib:install_element([enable_node, view_node], Show_lchan_summary_cmd),
    service_registration_lib:install_element([enable_node, view_node], Show_paging_cmd).
...
dump_all_bts(VTY)->
    NumBTS = ets:lookup_element(gsmnet_table, num_bts, 2),
    service_registration_lib:vty_out(VTY, "This is no real output!!!~n"),
...
dump_bts(VTY,BTS_NR)->
    service_registration_lib:vty_out(VTY, "BTS Number: ~p ", [BTS_NR]),
    service_registration_lib:vty_out(VTY, "is active~n").
...
net_dump_vty(VTY,  Gsmnet_table_name) ->	
    ...
    service_registration_lib:vty_out(VTY,  "BSC is on Country Code ~p, Network Code ~p "++
			   "and has ~p BTS~n", [CountryCode, NetworkCode, NumBTS]),  
    NameLong = ets:lookup_element(Gsmnet_table_name,name_long, 2),
    service_registration_lib:vty_out(VTY,  "  Long network name: ~p~n", [NameLong]),
    NameShort = ets:lookup_element(Gsmnet_table_name,name_short, 2),
    service_registration_lib:vty_out(VTY,  "  Short network name: ~p~n", [NameShort]),	
    AuthPolicy =  gsm_auth_policy_name(ets:lookup_element(Gsmnet_table_name,auth_policy, 2)),
    service_registration_lib:vty_out(VTY,  "  Authentication policy: ~p~n", [AuthPolicy]),
    RejectCause = ets:lookup_element(Gsmnet_table_name, reject_cause, 2),
    service_registration_lib:vty_out(VTY,  "  Location updating reject cause: ~p~n", [RejectCause]),
    A5Encryption =  ets:lookup_element(Gsmnet_table_name,a5_encryption, 2),
    service_registration_lib:vty_out(VTY,  "  Encryption: A5/~p~n", [A5Encryption]),
    Neci = ets:lookup_element(Gsmnet_table_name, neci, 2),
    service_registration_lib:vty_out(VTY,  "  NECI (TCH/H): ~p~n", [Neci]),
    PagAnyTCH = ets:lookup_element(Gsmnet_table_name, pag_amy_tch, 2),
    service_registration_lib:vty_out(VTY,  "  Use TCH for Paging any: ~p~n", [PagAnyTCH]),
    RrlpMode = ets:lookup_element(Gsmnet_table_name, rrlp_mode, 2),
    service_registration_lib:vty_out(VTY,  "  RRLP Mode: ~p~n", [RrlpMode]),
    SendMMInfo = case ets:lookup_element(Gsmnet_table_name, send_mm_info, 2) of
		     true -> "On";
		     false -> "Off"
		 end,
    service_registration_lib:vty_out(VTY,  "  MM Info: ~p~n", [SendMMInfo]),
    HandoverActive = case ets:lookup_element(Gsmnet_table_name, handover_active, 2) of
			 true -> "On";
			 false -> "Off"
		     end,
    service_registration_lib:vty_out(VTY,  "  Handover: ~p~n", [HandoverActive]),
    ChannelLoad = network_chan_load(Gsmnet_table_name),
    service_registration_lib:vty_out(VTY,  "  Current Channel Load: ~p~n", [ChannelLoad]),
	...
	true ->
	    ...
	    service_registration_lib:vty_out(VTY,  "  Last RF Command: ~p~n", [Msc_data_Rf_ctrl_Last_state_command]); 
	false ->
	    true
    end.
